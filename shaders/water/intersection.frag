...
bool intersectMaxMipmapAux(vec3 texOrigin, vec3 dir, out vec3 hitPos) {
...
bool intersectMaxMipmapAuxInvX(vec3 texOrigin, vec3 dir, out vec3 hitPos) {
...
bool intersectMaxMipmapAuxInvY(vec3 texOrigin, vec3 dir, out vec3 hitPos) {
...
bool intersectMaxMipmapAuxInvXY(vec3 texOrigin, vec3 dir, out vec3 hitPos) {
...
bool intersectMaxMipmap(vec3 rayOrigin, vec3 rayDir, out float pathLength, out vec3 hitPos) {
	...
	if (rd.x < 0 && rd.y < 0) {
		hit = intersectMaxMipmapAuxInvXY(ro, rd, hp);
    } else if (rd.x < 0) {
        hit = intersectMaxMipmapAuxInvX(ro, rd, hp);
    } else if (rd.y < 0) {
		hit = intersectMaxMipmapAuxInvY(ro, rd, hp);
    } else {
		hit = intersectMaxMipmapAux(ro, rd, hp);
	}
